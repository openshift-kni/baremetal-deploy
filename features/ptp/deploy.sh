...
deploy() {
    ...
    until oc -n openshift-ptp get csv -o yaml | sed "s,image-registry.openshift-image-registry.svc:5000/openshift/ose-ptp-operator@.*,${ptp_operator}:${channel},g" | sed "s,image-registry.openshift-image-registry.svc:5000/openshift/ose-ptp@.*,${ptp_daemon}:${channel},g" | oc apply -f -; do
        echo "failed to update csv images"
    done

    until oc -n openshift-ptp get deploy -o yaml | sed "s,image-registry.openshift-image-registry.svc:5000/openshift/ose-ptp-operator@.*,${ptp_operator}:${channel},g" | sed "s,image-registry.openshift-image-registry.svc:5000/openshift/ose-ptp@.*,${ptp_daemon}:${channel},g" | oc apply -f -; do
