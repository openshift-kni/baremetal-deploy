...
class BondingTemplator(object):
    def __init__(self, templates_dir: str):
        ...
        try:
            ...
            print("error loading templates directory", self.__templates_dir, ":", e)
            sys.exit(1)

    def __process_phy_devices(self, phy_devices: list, bond_device: str) -> list:
        results = list()
        for dev in phy_devices:
            try:
                ...
                dev["b64"] = base64.b64encode(output.encode("utf-8")).decode("utf-8")
    ...
    def __process_vlans(self, vlans: list, bond_device: str) -> list:
        results = list()
        for vlan in vlans:
            try:
                ...
                vlan["b64"] = base64.b64encode(output.encode("utf-8")).decode("utf-8")
                vlan["filename"] = (
                    "ifcfg-" + vlan.get("bond_device") + "." + str(vlan.get("id"))
                )
    ...
    def __generate_ifcfg_files(self, template_data: dict) -> dict:
        ...
        template_data["b64"] = base64.b64encode(bond_output.encode("utf-8")).decode(
            "utf-8"
        )
    ...
    def write_ignition_to_file(self, data: Text, outdir: str) -> NoReturn:
        outfile = outdir + "/" + str(random.randint(1, 10000)) + "-ignition-bond.ign"
        self.__write_template_to_file(data, outfile)
...
def main() -> NoReturn:
    ...
    parser.add_argument(
        "-f", "--param-file", required=True, type=str, help="path to parameters file"
    )
