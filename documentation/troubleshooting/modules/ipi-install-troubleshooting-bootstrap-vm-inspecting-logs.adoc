...
When experiencing issues downloading or accessing the RHCOS images, 
first verify that the URL is correct in the `install-config.yaml` 
configuration file.
...
The `ipa-downloader` and `coreos-downloader` containers download 
resources from a webserver or the external `https://quay.io` registry, 
whichever is specified in the `install-config.yaml` configuration file.
Verify the following two containers are up and running and inspect 
their logs as needed:
