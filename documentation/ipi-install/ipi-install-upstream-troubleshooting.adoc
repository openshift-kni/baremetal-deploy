...
:toc-title:
:imagesdir: images
:title: Troubleshooting Guide for IPI Installation
:description: Troubleshooting hints for debugging failures during IPI installation
:keywords: troubleshooting, OpenShift, baremetal, installation, Installer provisioned infrastructure
:prewrap!:
...
xref:ipi-install-troubleshooting-install-config_{context}[Troubleshooting `install-config.yaml] and xref:ipi-install-troubleshooting-post-installation-pod-errors_{context}[Post-installation Pod errors].
endif::[]

ifeval::[{product-version} != 4.3]
xref:ipi-install-troubleshooting-install-config_{context}[Troubleshooting `install-config.yaml].
endif::[]
...
include::modules/ipi-install-troubleshooting-ironic-bootstrap.adoc[leveloffset=+1]
