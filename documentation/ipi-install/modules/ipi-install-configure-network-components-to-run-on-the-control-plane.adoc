...
. Create a file named `cluster-network-99-config.yaml`
+
[source,bash]
----
$ touch cluster-network-99-config.yaml
----

. Open the `cluster-network-99-config.yaml` file in an editor and enter a custom resource (CR) that describes the Operator configuration:
+
[source,yaml]
----
...
. Save the `cluster-network-99-config.yml` file and quit the text editor.
