...
----
|
|The {product-title} cluster requires a name be provided for worker (compute) nodes even if there are zero nodes.
...
----
|
|Replicas sets the number of worker (compute) nodes in the {product-title} cluster.
...
| `disableCertificateVerification` | `False` | `redfish` and `redfish-virtualmedia` need this parameter set to `True` when using a self-signed certificate for BMC addresses.
...
| `address` | | The URL for communicating with the BMC controller, based on the provider being used. See the xref:bmcaddressing[BMC addressing section] for details.
|===
