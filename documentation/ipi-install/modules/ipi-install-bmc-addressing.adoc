...
[NOTE]
====
Currently RedFish is only supported on Dell with iDRAC firmware version `4.20.20.20` or higher for IPI on Bare metal deployments and from {product-title} version greater or equal to `4.6.1`.
====
